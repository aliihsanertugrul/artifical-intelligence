...
export default async function handler(request, response) {
  ...
  while (!generatedImages) {
    const results = await Promise.all(
      predictionsGets.map(async (element) => {
  ...
  response
    .status(200)
    .json(generatedImages ? generatedImages : "Failed to create the image");
}

//     let generatedImage;

//   while (!generatedImage) {
//     const result = await getGeneration(predictionsGets[0]);
//     console.log("result******",result)

//     if (result.status === "succeeded") {
//       [generatedImage] = result.output;
//       console.log("gener***",[generatedImage])
